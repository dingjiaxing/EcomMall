...
import com.jackting.module_main.widget.zxing.camera.PlanarYUVLuminanceSource;
...
final class DecodeHandler extends Handler {
  ...
  public void handleMessage(Message message) {
    ...
    if (message.what== R.id.main_decode){
      decode((byte[]) message.obj, message.arg1, message.arg2);
    }else if (message.what==R.id.main_quit){
...
//     PlanarYUVLuminanceSource source = CameraManager.get().buildLuminanceSource(data, height, width);
    ...
    if (rawResult != null) {
      ...
      Message message = Message.obtain(activity.getHandler(), R.id.main_decode_succeeded, rawResult);
      ...
      Message message = Message.obtain(activity.getHandler(), R.id.main_decode_failed);
      message.sendToTarget();
    }
  }
